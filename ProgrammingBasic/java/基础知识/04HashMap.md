...
　　基于哈希表的Map接口的实现。key和value都允许为null值。接近Hashtable（除了不是同步的，允许null值外）。不保证顺序。
...
final V putVal(int hash, K key, V value, boolean onlyIfAbsent, boolean evict) {
    ...
    //说明没有冲突，添加成功
    ++modCount;
    if (++size > threshold)//插入节点后，通过阈值进行扩容
        resize();
...
public class HashMap<K,V> extends AbstractMap<K,V>
    ...
    static class Node<K,V> implements Map.Entry<K,V> {
        ...
        public final V getValue()      { return value; }
        public final String toString() { return key + "=" + value; }
				//Obhects.hashCode方法是native修饰，底层用c++实现。该方法约定：
				//1.一致性：在程序一次执行过程中，同一个对象必须返回同一个整数。
				//2.Object类的　两个对象的equals相等，则hashCode的结果也应该一致
				//3.若对象通过java.lang.Object.equals(java.lang.Ojbect)比较，则不必保证两者结果一致。lang为运行时库
				//4.返回值不一定是内存地址，看ＪＶＭ具体实现和参数。hashCode底层有多种方法实现
				// 参考：http://blog.csdn.net/god8816/article/details/53866182
				//此方法　返回key value哈希值的　异或
     ...
     */
    //计算key的hash值，　hashCode值的高位也参与计算
    static final int hash(Object key) {
        int h;
        return (key == null) ? 0 : (h = key.hashCode()) ^ (h >>> 16);//无符号右移１６
    }
    ...
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
                   boolean evict) {
        Node<K,V>[] tab; Node<K,V> p; int n, i;
        //判断表是否为空，若为空，则创建一个新的table数组，并且获取该数组的长度，若不为空，则获取长度n
        if ((tab = table) == null || (n = tab.length) == 0)
            n = (tab = resize()).length;
        ...
        if ((p = tab[i = (n - 1) & hash]) == null)
            ...
            if (p.hash == hash &&
                ((k = p.key) == key || (key != null && key.equals(k))))
                e = p;　//返回原节点e
            //判断table[i] 是否为treeNode红黑树，如果是红黑树，则直接在树中插入键值对
            else if (p instanceof TreeNode)
                e = ((TreeNode<K,V>)p).putTreeVal(this, tab, hash, key, value);
            //该链为链表
            else {
                //遍历table[i]，判断链表长度是否大于TREEIFY_THRESHOLD(默认值为8)，大于8的话把链表转换为红黑树，在红黑树中执行插入操作，否则进行链表的插入操作
                for (int binCount = 0; ; ++binCount) {　//binCount　bin节点个数
                    if ((e = p.next) == null) {//如果当前节点为最后一个节点
                        p.next = newNode(hash, key, value, null);
                        if (binCount >= TREEIFY_THRESHOLD - 1) // -1 for 1st　　超过８个bins
                            treeifyBin(tab, hash); //转为红黑树
                        break;
                    }
                    if (e.hash == hash &&　//如果当前节点有相同的key，则跳出循环
                        ((k = e.key) == key || (key != null && key.equals(k))))
                        break;
                    p = e;　//继续循环
                }
            }
            if (e != null) { // existing mapping for key　　写入相同的key
                V oldValue = e.value;
    ...
    final Node<K,V>[] resize() {
        ...
        int newCap, newThr = 0;
        if (oldCap > 0) {
            if (oldCap >= MAXIMUM_CAPACITY) {　//已经为最大容量了，直接返回原来的数组
